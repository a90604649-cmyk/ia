...
const PROGRAMMER_SYSTEM =
    ...
    "- Si existe un archivo con el mismo path + name en el manifest y el ClassName coincide, la acción obligatoriamente debe ser update_*.\n" +
    "- Si no existe en el manifest, entonces puede ser create_*.\n" +
...
function esTareaDeCodigo(mensaje) {
    const patrones = [
        ...
        "problema", "resolver", "resuelve", "arregla", "arreglar",
        "agrega", "agregar", "añade", "añadir", "ponle", "poner",
        "quitar", "quita", "cambia", "cambiar", "implementa", "implementar",
        "vuelo", "volar", "volando", "tecla", "teclas", "key", "input",
        "log", "logs", "evento", "eventos", "detecta", "detectar",
        "funciona", "funcionando", "no funciona", "no sirve",
        "localscript", "local script", "modulescript", "module script",
        "remoteevent", "remotefunction", "gui", "tool", "remote",
        "velocidad", "sprint", "salto", "correr", "stamina",
        "f", "w", "a", "d", "space", "shift"
    ];

    const texto = String(mensaje || "").toLowerCase();

    const tienePeticionDeCambio = [
        "modifica", "modificar", "actualiza", "actualizar", "corrige", "corregir",
        "resuelve", "resolver", "arregla", "arreglar", "agrega", "agregar",
        "añade", "añadir", "ponle", "poner", "cambia", "cambiar", "crea", "crear",
        "elimina", "eliminar", "borra", "borrar", "haz", "hacer", "implementa", "implementar"
    ].some((patron) => texto.includes(patron));

    const tieneContextoRoblox = [
        "roblox", "studio", "script", "localscript", "modulescript", "luau", "lua",
        "sistema", "vuelo", "volar", "tecla", "input", "gui", "remote", "sprint",
        "stamina", "tool", "animacion", "animación"
    ].some((patron) => texto.includes(patron));

    if (tienePeticionDeCambio && tieneContextoRoblox) {
        return true;
    }
...
async function hablarConGemini(mensajeUsuario) {
    ...
    for (let intento = 1; intento <= 2; intento++) {
        try {
            ...
            if (!temporal || intento === 2) return null;
...
async function prepararTareaConGemini(mensajeUsuario) {
    ...
    try {
        ...
        if (!brief) throw new Error("Gemini no generó el brief técnico.");
...
function normalizarTipoGenerico(action) {
    ...
    if (!operation || !["create", "update", "delete"].includes(operation)) return action.type;
...
function normalizarAccion(action) {
    ...
    if (!TIPOS_CRUD.has(type)) return null;
    ...
    const rutaValida = rutasPermitidas.some((base) => path === base || path.startsWith(`${base}/`));
    if (!name || !rutaValida || path.includes("..")) return null;
    ...
    if (suppliedClass && suppliedClass !== className) return null;

    const result = { type, className, name, path };
...
function extraerResultado(texto) {
    ...
    if (!objeto || !Array.isArray(objeto.actions)) return null;

    const actions = [];
    for (const action of objeto.actions) {
        const normalizada = normalizarAccion(action);
        if (normalizada) actions.push(normalizada);
        else console.warn("⚠️ Acción rechazada durante validación:", action?.type || action?.operation || "desconocida", action?.path || "", action?.name || "");
    }
...
async function ajustarAccionesCRUDPorProyecto(resultado) {
    if (!resultado || !Array.isArray(resultado.actions)) return resultado;

    try {
        const contextPort = Number(process.env.PROJECT_CONTEXT_PORT || 3001);
        const response = await fetch(`http://127.0.0.1:${contextPort}/project-summary`);
        ...
        for (const script of manifest) {
            if (!script?.path || !script?.name || !script?.className) continue;
        ...
        const actions = resultado.actions.map((action) => {
            if (!action?.path || !action?.name) return action;
            if (action.type === "create_folder" || action.type === "delete_folder") return action;
            ...
            if (!existente) {
                if (
                    ...
                    corregidas += 1;
                    console.log(`🔧 CRUD: ${key} no existe; update_* → ${tipoCreatePorClase[action.className]}.`);
                    return { ...action, type: tipoCreatePorClase[action.className] };
                }
                return action;
            }
            ...
            const esCreate = ["create_script", "create_local_script", "create_module_script"].includes(action.type);
            const esUpdate = ["update_script", "update_local_script", "update_module_script"].includes(action.type);

            if (esCreate && action.className === claseReal) {
                corregidas += 1;
                console.log(`🔧 CRUD: ${key} ya existe como ${claseReal}; create_* → ${tipoUpdate}.`);
                return { ...action, type: tipoUpdate, className: claseReal };
            }

            if (esUpdate && action.className && action.className !== claseReal) {
                corregidas += 1;
                console.log(`🔧 CRUD: ${key} tiene ClassName real ${claseReal}; corrigiendo ${action.type} → ${tipoUpdate}.`);
                return { ...action, type: tipoUpdate, className: claseReal };
            }
        ...
        if (corregidas > 0) console.log(`✅ CRUD: ${corregidas} acción(es) ajustada(s) usando el manifest real del proyecto.`);
        return { ...resultado, actions };
    } catch (error) {
        console.warn("⚠️ No se pudo validar CRUD contra Project Context:", error instanceof Error ? error.message : String(error));
        return resultado;
    }
}
    ...
    for (let intento = 1; intento <= 2; intento++) {
        try {
            ...
            if (!resultado) {
                ...
                instruccion =
                    "La respuesta anterior no pudo ser interpretada. Devuelve ÚNICAMENTE un objeto JSON válido con reply y actions. " +
                    "Cada acción debe usar exactamente uno de estos tipos: " + Array.from(TIPOS_CRUD).join(", ") + ". " +
                    "En update/create usa el ClassName real del contexto y el SOURCE COMPLETO. En delete no incluyas code.\n\n" +
                    "PETICIÓN ORIGINAL:\n" + mensajeUsuario + "\n\nBRIEF:\n" + brief + contextoScript;
                continue;
            }
            ...
            agregarHistorial("user", mensajeUsuario);
            agregarHistorial("model", resultado.reply || "Sistema programado con DeepSeek.");
...
async function preguntar(mensajeUsuario) {
    ...
    if (esTareaDeAnimacion(texto)) {
        try {
            ...
            const resultado = await procesarAnimacion(texto, ai, historial, r6Calibration);
            if (!resultado) {
    ...
    if (esTareaDeCodigo(texto)) {
        console.log("🛠️ Router: petición detectada como tarea de programación; usando DeepSeek.");
...
app.post("/r6-calibration", function(req, res) {
    try {
        const body = req.body;
        if (!body || typeof body !== "object") return res.status(400).json({ ok: false, error: "Calibración inválida." });
        if (body.rig !== "R6") return res.status(400).json({ ok: false, error: "La calibración no corresponde a R6." });
        if (!body.rigName || !body.joints || typeof body.joints !== "object") return res.status(400).json({ ok: false, error: "Datos de calibración incompletos." });
        r6Calibration = body;
...
app.post("/selected-script", function(req, res) {
    try {
        ...
        if (!tiposPermitidos.has(body.className) || typeof body.name !== "string" || typeof body.path !== "string" || typeof body.source !== "string") {
...
app.listen(PORT, function() {
    ...
    console.log("Router: peticiones de programación detectadas → DeepSeek");
    console.log("=================================\n");
});
